...
def main():
    ...
    if len(sys.argv) > 1:
        ply_path = sys.argv[1]
        print(f"PLY file argument received: {ply_path}")
        print("Note: PLY rendering is disabled in minimal mode")
    ...
    window = glfw.create_window(width, height, "Minimal OpenGL 3.3 Grid Demo", None, None)
    if not window:
        glfw.terminate()
        return
