...
def transform_vertices(vertices, mvp_matrix):
    ...
    # Store w values before division for clipping check
    w_values = transformed[:, 3]
    
    # Avoid division by zero
    w_values = np.where(np.abs(w_values) < 0.00001, 0.00001, w_values)
    
    # Divide x, y, z by w
    transformed[:, 0] /= w_values
    transformed[:, 1] /= w_values
    transformed[:, 2] /= w_values
    ...
    # Mark vertices that are behind the camera (w < 0) for clipping
    # In a real implementation, you'd want proper frustum clipping
    valid_mask = w_values > 0.001
