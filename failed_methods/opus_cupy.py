...
def transform_vertices(vertices, mvp_matrix, use_cupy=True):
    ...
    if use_cupy and CUPY_AVAILABLE:
        # Transfer to GPU
        positions_4d_gpu = cp.asarray(positions_4d)
        mvp_matrix_gpu = cp.asarray(mvp_matrix)
        
        # Transform by MVP matrix on GPU
        transformed_gpu = cp.dot(positions_4d_gpu, mvp_matrix_gpu.T)
        
        # Transfer back to CPU for the rest of processing
        transformed = cp.asnumpy(transformed_gpu)
    else:
        # Transform by MVP matrix on CPU
...
def main():
    ...
    # Main loop
    print(f"Using {'CuPy' if CUPY_AVAILABLE else 'NumPy'} for matrix multiplication")
    while not glfw.window_should_close(window):
        ...
        transformed_vertices, valid_mask = transform_vertices(original_vertices, mvp, use_cupy=True)
